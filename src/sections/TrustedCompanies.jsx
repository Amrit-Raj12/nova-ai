    ...
    <section className="border-b border-line bg-white dark:border-[#293734] dark:bg-[#141d1b]">
      <Container>
        <div className="py-8 sm:py-10">
          <p className="text-center text-xs font-medium text-slate dark:text-gray-500">
              ...
              {[0, 1].map((group) => (
                ...
                >
                  {companies.map((company) => (
                    <div
                      key={`${group}-${company}`}
                      className="w-24 shrink-0 text-center text-sm font-bold tracking-tight text-[#7A858D] transition hover:text-ink dark:text-gray-600 dark:hover:text-gray-300 sm:w-28"
                    >
                      {company}
                    </div>
                  ))}
